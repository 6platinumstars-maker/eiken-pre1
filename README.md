...
- `単語チェック` モード
  - 最初は英単語のみ表示
  - 1回タップで日本語の意味と単語カードを表示
  - 2回目のタップでその単語をチェック済みに変更
  - 3回目のタップでその単語のチェックを解除
  - 展開した単語カードに `品詞` を表示
  - 動詞は `自動詞 / 他動詞 / 自他両用 / 未確認` の区分を表示
  - `再生` で女性2回・男性2回・女性1回の単語音声を再生
  - 音声終了後は次の単語へ自動で進み、Section末尾で停止
- `単語復習` モード
  - チェック済み単語をすべて一覧表示
  - 1つのボタンで `再生` / `停止` を切り替え（一時停止・再開はなし）
  - 再生ボタンと同じ行の `すべての単語カードを表示する` / `すべての単語カードを閉じる` でチェック済みカードを一括開閉
  - 再生開始時のチェック済み単語を全セクションから番号順に再生し、最後で停止
  - 再生中の単語カードを操作ボタン直下へ自動スクロール。操作ボタンは上部に固定し、最後のカードも揃うよう下部に余白を確保
  - 各単語は既存の5回連続音声（女性2回・男性2回・女性1回）を使用
  - 再生待ちの単語のチェックを外すと、その単語をスキップ。再生中の単語は最後まで再生
  - 再生途中にチェックを追加した単語は、次回の再生開始時に反映
  - 全チェック解除・画面切り替え・Section変更で停止。停止後の再生は一覧の先頭から開始
  - 単語カードのタップで、訳・追加情報・例文と訳を開閉
  - カードの開閉ではチェック状態を変更しない
  - チェック欄を直接操作した場合のみ、個別にチェック状態を変更可能
